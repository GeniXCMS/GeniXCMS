    ...
    self.each(function(baseIndex, baseElem) {
        ...
        function setup() {
            ...
            var btnGroup = $('<div class="btn-group pull-right"/>')
                .appendTo(wrapper)
            ;
            var saveButton = $('<button title="Save Code" type="button" class="btn btn-xs btn-success gm-edit-mode"><span class="glyphicon glyphicon-floppy-disk"></span></button>')
                .on('click', function() {
                    var html = $('#myGrid').gridEditor('getHtml');
                    htmlTextArea.val(html);
                    $('.note-editable').html(html);

                    // saveButton.toggleClass('active btn-danger');
                })
                .appendTo(btnGroup)
            ;
