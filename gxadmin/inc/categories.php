...
if ($data['num'] > 0 && !isset($data['cat']['error'])) {
    foreach ($data['cat'] as $c) {
        $c = (object)$c;
        if (!is_object($c) || isset($c->error)) continue;
                    ...
                    $subHtml .= "
                        <div class='badge bg-light text-dark fw-medium rounded-pill px-3 py-2 border d-flex align-items-center gap-2 group-badge'>
                            ...
                            <div class='badge-actions'>
                                ...
                                <a href='?page=categories&act=del&id={$c2->id}&token=".TOKEN."' class='text-danger ps-1' onclick=\"return confirm('"._("Delete this sub-category?")."');\"><i class='bi bi-dash-circle'></i></a>
                            </div>
                        </div>";
                }
            }
                    ...
                    <div class='d-flex align-items-start gap-4'>
                        ...
                        <div class='flex-grow-1 min-w-0'>
                            ...
                            <div class='d-flex align-items-center gap-2'>
                                ...
                                <a href='?page=categories&act=del&id={$c->id}&token=".TOKEN."' class='btn btn-sm btn-white text-danger border rounded-circle p-1' onclick=\"return confirm('"._("Are you sure?")."');\"><i class='bi bi-trash'></i></a>
                            </div>
                        </div>
                    </div>
...
$schema = [
    ...
    'content' => [
        ...
        ['type' => 'row', 'items' => (function() use ($categoryCards) {
            $items = [];
        ...
        [
            ...
            'body_elements' => [
                [
                    'type' => 'form',
                    'action' => 'index.php?page=categories',
                    'fields' => [
                        ['type' => 'row', 'items' => [
                            ['width' => 7, 'content' => ['type' => 'raw', 'html' => '
                                <div class="mb-4">
                                ...
                                <div class="mb-4">
                                    ...
                                    '.Categories::dropdown(['parent' => '0', 'name' => 'parent', 'sort' => 'ASC', 'order_by' => 'name', 'type' => 'post', 'class' => 'form-select border-0 bg-light rounded-3 py-2 px-3']).'
                                </div>'
                            ]],
                            ['width' => 5, 'content' => ['type' => 'raw', 'html' => '
                                <div class="mb-4">
                        ...
                        ['type' => 'row', 'items' => [
                            ['width' => 7, 'content' => ['type' => 'raw', 'html' => '
                                ...
                                <div class="mb-4">
                                    ...
                                    '.Categories::dropdown(['parent' => '0', 'name' => 'parent', 'id' => 'edit_cat_parent', 'sort' => 'ASC', 'order_by' => 'name', 'type' => 'post', 'class' => 'form-select border-0 bg-light rounded-3 py-2 px-3']).'
                                </div>'
                            ]],
                            ['width' => 5, 'content' => ['type' => 'raw', 'html' => '
...
<style>
    .category-card { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid #f1f5f9 !important; }
    .category-card:hover { transform: translateY(-5px); box-shadow: 0 20px 40px rgba(15, 23, 42, 0.08) !important; border-color: rgba(59, 130, 246, 0.2) !important; }
    .category-icon-box { width: 64px; height: 64px; font-size: 24px; }
    .ls-1 { letter-spacing: 0.5px; }
    .fs-xs { font-size: 0.75rem; }
    .line-clamp-2 { display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
    .group-badge { transition: all 0.2s; position: relative; overflow: hidden; }
    .badge-actions { display: none; margin-left: 8px; font-size: 11px; }
    .group-badge:hover .badge-actions { display: inline-flex; align-items: center; }
</style>
