...
$schema = [
    'header' => [
        ...
        'button' => [
            'type' => 'button',
            'label' => _('New Language'),
            'icon' => 'bi bi-plus-lg',
            'btn_type' => 'button',
            'class' => 'btn btn-primary rounded-pill px-4 shadow-sm fw-bold',
            'attr' => 'data-bs-toggle="modal" data-bs-target="#addcountry"'
        ],
    ],
    'card_wrapper' => false,
    'content' => [
        ...
        <div class="modal-content border-0 shadow-lg rounded-5 overflow-hidden">
            <form action="index.php?page=multilang" method="post">
                <div class="modal-header border-0 py-4 px-5">
                    <h5 class="modal-title fw-black text-dark text-uppercase tracking-widest fs-7">
                        <?= _("Language Architect"); ?>
                    </h5>
                ...
                <div class="modal-body p-5 pt-0">
                    <div class="row g-4">
                        <div class="col-12">
                            <label class="form-label fw-black text-muted extra-small text-uppercase tracking-wider"
                                style="font-size:0.65rem;"><?= _("Visual Display Name"); ?></label>
                            <input type="text" name='multilang_country_name'
                                class="form-control border bg-light rounded-4 py-3 px-3 shadow-none fs-8 fw-bold"
                                placeholder="e.g. English">
                            <div class="extra-small text-muted mt-2 fw-medium">
                                <?= _("Human readable name for this localization."); ?>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-black text-muted extra-small text-uppercase tracking-wider"
                                style="font-size:0.65rem;"><?= _("ISO Identifier"); ?></label>
                            <input type="text" name="multilang_country_code"
                                class="form-control border bg-light rounded-4 py-3 px-3 shadow-none fs-8 fw-bold"
                                placeholder="en">
                            <div class="extra-small text-muted mt-2 fw-medium"><?= _("Lowercase (e.g. en, id)."); ?>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-black text-muted extra-small text-uppercase tracking-wider"
                                style="font-size:0.65rem;"><?= _("Flag Icon"); ?></label>
                            <select name="multilang_country_flag"
                                class="form-select border bg-light rounded-4 py-3 px-3 shadow-none fs-8 fw-bold">
                                <?= Date::optCountry(); ?>
                            </select>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-black text-muted extra-small text-uppercase tracking-wider"
                                style="font-size:0.65rem;"><?= _("System Mapping Protocol"); ?></label>
                            <select name="multilang_system_lang"
                                class="form-select border bg-light rounded-4 py-3 px-3 shadow-none fs-8 fw-bold">
                                <?= Language::optDropdown(); ?>
                            </select>
                            <div class="extra-small text-muted mt-2 fw-medium">
                                <?= _("Maps to internal system translation files."); ?>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-0 p-5 pt-0">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm fw-bold"
                        name="addcountry"><?= _("Create Dialect"); ?></button>
                </div>
