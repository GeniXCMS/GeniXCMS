...
// Stats Data
$statsItems = [
    ['label' => _('Total Library'), 'value' => (string) Stats::totalPost($data['postType']), 'icon' => 'bi bi-journal-album', 'color' => 'primary'],
    ['label' => _('Live Assets'), 'value' => (string) Stats::activePost($data['postType']), 'icon' => 'bi bi-broadcast-pin', 'color' => 'success'],
    ['label' => _('Archived / Draft'), 'value' => (string) Stats::inactivePost($data['postType']), 'icon' => 'bi bi-archive', 'color' => 'warning']
];
...
$schema = [
    ...
    'content' => [
        ...
        [
            'type' => 'card',
            'title' => _('Records Library'),
            'icon' => 'bi bi-database-fill',
            'no_padding' => true,
            'header_action' => '
                <form action="index.php?page=posts" method="get" class="d-flex gap-2 flex-wrap justify-content-end align-items-center">
                    <input type="hidden" name="page" value="posts">
                    <div class="input-group input-group-sm w-auto shadow-sm rounded-pill overflow-hidden border">
                        <span class="input-group-text bg-white border-0 ps-3"><i class="bi bi-search text-muted"></i></span>
                        <input type="text" name="q" class="form-control border-0 ps-1 bg-white" placeholder="' . _("Search...") . '" style="width:140px;" value="' . ($_GET['q'] ?? '') . '">
                    </div>
                    ...
                    ' . Categories::dropdown(['name' => 'cat', 'type' => $data['postType'], 'class' => 'form-select form-select-sm rounded-pill px-3 shadow-none border bg-white shadow-sm', 'selected' => ($_GET['cat'] ?? ''), 'attr' => 'style="width:130px;"']) . '
