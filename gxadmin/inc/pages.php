...
$schema = [
    'header' => [
        ...
        'button' => [
            'type' => 'link',
            'href' => 'index.php?page=pages&act=add&token=' . TOKEN,
            'label' => _('New Page'),
            'icon' => 'bi bi-plus-lg',
    ...
    'content' => [
        ...
        [
            ...
            'header_action' => '
                <form id="pages-filter-form" class="d-flex gap-2 flex-wrap justify-content-end align-items-center" onsubmit="loadPages(); return false;">
                    <div class="input-group input-group-sm w-auto shadow-sm rounded-pill overflow-hidden border">
                        <span class="input-group-text bg-white border-0 ps-3"><i class="bi bi-search text-muted"></i></span>
                        <input type="text" name="q" class="form-control border-0 ps-1 bg-white" placeholder="' . _("Locate page...") . '" style="width:160px;" value="' . Typo::cleanX($_GET['q'] ?? '') . '">
                    </div>
                    ...
                    <button type="button" class="btn btn-sm btn-outline-primary rounded-pill px-3" onclick="loadPages()"><i class="bi bi-arrow-clockwise me-1"></i> Refresh</button>
                </form>',
            'body_elements' => [
                [
                    'type' => 'raw',
                    'html' => '
                        <form action="" method="post" id="pages-bulk-form">
                            <div id="pages-desktop-container" class="p-0">
            ...
            'footer' => '
                <div class="d-flex justify-content-between align-items-center w-100 p-3">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <div id="pages-pagination-info" class="small text-muted fw-bold"></div>
                        <nav id="pages-pagination-nav"></nav>
                    </div>
                </div>'
        ]
    ]
...
// Allow developers to modify the entire pages dashboard schema
$schema = Hooks::filter('admin_pages_schema', $schema);
...
<script>
    ...
    function loadPages(offset = 0) {
        const container = $('#pages-desktop-container');
        const filterData = $('#pages-filter-form').serialize();
        const ajaxUrl = '<?= Url::ajax("posts", "list_posts") ?>&type=page&offset=' + offset + '&' + filterData;
        ...
        $.getJSON(ajaxUrl, function(response) {
            if (response.status === 'success') {
                renderPagesTable(response.headers, response.data);
                renderPagesPagination(response.total, response.limit, response.offset);
            } else {
    ...
    function renderPagesTable(headers, data) {
        const container = $('#pages-desktop-container');
        if (data.length === 0) {
    ...
    function renderPagesPagination(total, limit, offset) {
        const info = $('#pages-pagination-info');
        const nav = $('#pages-pagination-nav');
        ...
        paginationHtml += `
            <li class="page-item ${currentPage === 1 ? 'disabled' : ''}">
                <a class="page-link" href="#" onclick="loadPages(${(currentPage - 2) * limit}); return false;"><i class="bi bi-chevron-left"></i></a>
            </li>
        `;
            ...
            if (i === 1 || i === totalPages || (i >= currentPage - 2 && i <= currentPage + 2)) {
                paginationHtml += `
                    <li class="page-item ${i === currentPage ? 'active' : ''}">
                        <a class="page-link" href="#" onclick="loadPages(${(i - 1) * limit}); return false;">${i}</a>
                    </li>
                `;
        ...
        paginationHtml += `
            <li class="page-item ${currentPage === totalPages ? 'disabled' : ''}">
                <a class="page-link" href="#" onclick="loadPages(${currentPage * limit}); return false;"><i class="bi bi-chevron-right"></i></a>
            </li>
        `;
