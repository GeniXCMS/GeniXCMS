...
<div class="row mb-4">
    <div class="col-md-12 text-center">{Gneex::opt('adsense')|noescape}</div>
</div>
{/if}
...
<aside class="sidebar-portal">
    ...
    <div class="card sidebar-card mb-5 border-0 shadow-sm overflow-hidden bg-white" data-aos="fade-left">
        <div class="card-header bg-white border-0 pt-4 px-4 pb-0">
            <h3
                class="card-title premium-sidebar-title fs-6 fw-bold text-uppercase m-0 d-flex align-items-center gap-2">
            ...
            <div class="sidebar-item d-flex align-items-center mb-4 transition-base">
                <div class="flex-shrink-0">
                    <a href="{Url::post($r->id)}">
                        <div class="sidebar-img-wrapper rounded-3 overflow-hidden shadow-sm"
                            style="width: 70px; height: 70px;">
                            <img src="{$rp_img ? Url::thumb($rp_img, 'square', '100') : Url::thumb(Site::$url . 'assets/images/noimage.png', 'square', '100')|noescape}"
                ...
                <div class="flex-grow-1 ms-3 overflow-hidden">
                    <h4 class="h6 mb-2 sidebar-post-title line-clamp-2">
                        <a href="{Url::post($r->id)}"
                            class="text-dark text-decoration-none fw-bold fs-7 lh-base">{$r->title}</a>
                    </h4>
    ...
    <div class="card sidebar-card mb-5 border-0 shadow-sm overflow-hidden bg-white" data-aos="fade-left"
        ...
        <div class="card-body p-4 pt-3">
            {var $comments = Db::result("SELECT * FROM `comments` WHERE `status` = '1' AND `type` = 'post' ORDER BY
            `date` DESC LIMIT 5")}
            {if is_array($comments) && !isset($comments['error'])}
            {foreach $comments as $c}
            {var $c_author = !empty($c->userid) ? $c->userid : $c->name}
            <div
                class="sidebar-comment-item mb-3 pb-3 border-bottom border-light-subtle last-child-border-0 transition-base">
                ...
                <div
                    class="extra-small text-muted fst-italic ps-3 border-start border-primary border-opacity-10 ms-1 line-clamp-2">
    ...
    <div class="card sidebar-card mb-5 border-0 shadow-sm overflow-hidden bg-white" data-aos="fade-left"
        ...
        <div class="card-body p-4 pt-4">
            <div class="archive-widget-list">
                {Archives::getList(10)|noescape}
            </div>
        </div>
    </div>
    {/if}
